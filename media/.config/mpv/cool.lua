...
local config = {
	...
	model_name = "video-subs", -- Tools -> Manage note types
	sentence_field1 = "SourceText1",
	sentence_field2 = "SourceText2",
	video_field = "SourceAudio",
	menu_font_size = 25,
	note_tag = "dual-sub", -- the tag that is added to new notes. change to "" to disable tagging
...
local function construct_note_fields(sub_text1, sub_text2, video_filename)
	if not is_empty(sub_text1) then
		sub_text1 = trim(sub_text1)
	end
	if not is_empty(sub_text2) then
		sub_text2 = trim(sub_text2)
	end
	return {
		[config.sentence_field1] = sub_text1, -- Still [sound:%s] for compatibility with anki
		[config.sentence_field2] = sub_text2, -- Still [sound:%s] for compatibility with anki
...
local function export_to_anki(gui)
	...
	local note_fields = construct_note_fields(sub1['text'], sub2['text'], video_filename)
...
local function init_platform_nix()
	local self = {}
	local clip = is_running_macOS() and 'LANG=en_US.UTF-8 pbcopy' or 'echo $WAYLAND_DISPLAY' and 'wl-copy' or 'xclip -i -selection clipboard'
...
subs.get_current = function()
	local sub_text1 = mp.get_property("sub-text")
	local sub_text2 = mp.get_property("sub-text")
	if not is_empty(sub_text1) or not is_empty(sub_text2) then
...
subs.get_text = function()
	...
	for _, sub in ipairs(subs.list) do
		table.insert(speech1, sub['text1'])
	end
	for _, sub in ipairs(subs.list) do
		table.insert(speech2, sub['text2'])
	end
	return table.concat(speech1, ' '), table.concat(speech2, ' ')
end
...
subs.append = function()
	local sub1, sub2 = subs.get_current()

	if sub1 ~= nil and not table.contains(subs.list, sub1) then
		table.insert(subs.list, sub1)
		menu.update()
	end
	if sub2 ~= nil and not table.contains(subs.list, sub2) then
		table.insert(subs.list, sub2)
		menu.update()
	end
end
